...
static const char * const cpwm_register_names[] = {
 ...
 * increments or decrements continuously between 0 and the value of the
 * Interval register, with the direction of counting determined by the DEC bit
 * of the Counter Control register. An interval interrupt is generated when the
 * counter passes through zero. The corresponding match interrupt is generated
 * when the counter value equals one of the Match registers." [UG585]
 */

struct cadence_pwm_pwm {
	u32 clk_hz; /* Per-PWM clock frequency */
...
static inline volatile __iomem u32 *cpwm_register_address(
	struct cadence_pwm_chip *cpwm, int pwm, enum cpwm_register reg)
{
	return (u32 *)(4 * (3 * reg + pwm) +
		(char *)cpwm->base);
}

static u32 cpwm_read(struct cadence_pwm_chip *cpwm, int pwm,
...
static void cpwm_write(struct cadence_pwm_chip *cpwm, int pwm,
		       enum cpwm_register reg, u32 value)
{
	dev_dbg(cpwm->chip.dev,
			      ...
			      int duty_ns, int period_ns)
{
	...
	period_clocks = div64_u64(((int64_t)period_ns *
				   (int64_t)cpwm->pwms[h].clk_hz),
				  1000000000LL);
	...
	duty_clocks = div64_u64(((int64_t)duty_ns *
				 (int64_t)cpwm->pwms[h].clk_hz),
				1000000000LL);
