...
/* sysfs interface */
static ssize_t abrtcmc_sysfs_show_temp(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	...
	if (ret)
		return ret;
	data -= 60;
	return sprintf(buf, "RTC temperature is : %d\n", data);
}

static DEVICE_ATTR(temp, 0444, abrtcmc_sysfs_show_temp, NULL);
